...
## G8 four-bit representations and the placement clock

The [G8 protocol](../results/historical/g8-coreml-four-bit/protocol.json) records the representations, phases and clock mappings. The Core ML exporter builds each form from the same codes and FP16-rounded scales; the saved package is read back without loading the model, and its audit records the constant-expression operators and, per projection, the hashes of the codes and of the decoded FP16 weights. The verifier requires each form's expected operators, codes equal to the reference inputs' codes, and one decoded-weight set shared by the four Core ML forms at both sizes. The Core AI asset is G7's export.

Controls, the numeric, zero and repeat rules, warmup, timing, energy integration and block admission are those of [G7](#g7-same-codes-in-two-runtimes), with a single 0.005 limit. Placement counts successful direct ANE requests inside each control call window, ±100 µs. The original phase placed each log line on the host clock through its microsecond wall-clock timestamp and an epoch/monotonic pair read at host start. The follow-up read Mach continuous/absolute bridges before and after the controls and mapped each log line's continuous ticks onto the host's absolute clock; the mapped interval, bridge uncertainty included, must lie inside the window. The follow-up timed exactly the rejected first rounds and their cancelled later rounds, with the original job IDs, seeds and order.

The importer recounts every admission from the host-PID log under both mappings and refuses a count that differs from the record under the phase's own mapping; every original host uses one bridge read during the original run. The verifier applies the run's rule under each phase's mapping. Beyond the run's rules, and named as such, it requires the Mach-clock recount to find a request in every control call of every admission and block, every non-constant Core ML operation to prefer the Neural Engine, and each block to show ANE power above 1 W and GPU power below 0.05 W. Ratios pair blocks of the same round and are labelled when they cross phases. Output classes group representations whose control-output hashes are equal for every input. The comparison with G7 divides G8's median speed by G7's for the configurations stored the same way in both.
