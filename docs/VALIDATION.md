...
description and project-name edits on 2026-09-11 and the author change on 2026-09-15. `uv.lock` also differs from those
...
## G6 import — 2026-09-14

The G6 importer ran twice into new directories and produced byte-identical bundles. The verifier recomputes <!-- claim:g6.requests@g6-001 -->898<!-- /claim --> request results, <!-- claim:g6.power-samples@g6-002 -->13,774<!-- /claim --> power samples in <!-- claim:g6.captures@g6-003 -->4<!-- /claim --> captures and <!-- claim:g6.blocks@g6-004 -->42<!-- /claim --> blocks, and compares every rate, energy field, admission and idle power with the recorded summary; repeat ratios read the G4 A bundle. Disposable-copy tests change a product, a timed request's query width, an ANE request count in a W4 or FP16 admission log, the ANE energy of a W4 power frame together with its decoded record, a recorded speed, a repeat prefill count, a recorded idle power and a tier's graph inventory, and each fails by name. No device was run for this import.

The short decode query verifier checks every case's function inventory and input signatures, host return code, ANE log counts, MPSGraph assertion and same-logits comparison. Disposable-copy tests make a width-4 host fail, remove a width-1 driver status and change an archived script, and each fails by name. The palettized placement verifier checks that the pair's export records match except for compression, one compile result per bundle, the W4 compile failure with no ANE requests, the FP16 ANE requests and equal logits across widths; disposable copies add W4 ANE requests or change a capacity, and each fails by name.

## G7 import and the Core ML QDQ probe — 2026-09-15

Both importers ran twice into new directories and produced byte-identical outputs. The G7 importer recounts every control call's ANE requests from the host-PID logs, checks each block's clock order, count and recorded p50, and refuses a mismatch. The verifier re-applies the numeric, zero, repeat and placement rules to all <!-- claim:g7.configs@g7-077 -->42<!-- /claim --> prepared configurations and to every block's own controls, recomputes <!-- claim:g7.blocks@g7-078 -->78<!-- /claim --> blocks from their windows, call counts and <!-- claim:g7.power-samples@g7-079 -->12,353<!-- /claim --> power samples, and compares speed, latency, energy at four lags, admission and idle power with what the run recorded. Disposable-copy tests change an A8 relative L2, a zero-output hash, an ANE request count, a projection's and another Core ML operation's preferred device, the ANE energy of a power frame together with its decoded record, a recorded speed and a recorded idle power, and each fails by name. The Core ML probe verifier reads all 96 outputs; disposable copies replace an unclamped Neural Engine output with the correct value, remove an ANE request and delete a clamp from a graph, and each fails by name. No device was run for this import.

## Hash reduction — 2026-09-15

The bundle-product, archived-script and figure-hash checks were removed with the hashes they compared ([what was kept and why](PROVENANCE.md#hash-reduction--2026-09-15)); their tests in the dated sections above no longer exist. `render_figures.py --check` now regenerates every figure into a temporary directory and compares the bytes; in a disposable copy, a changed SVG and a changed manifest each fail by name. G2's equal-work check compares the C and G inputs and arrival offsets directly. G6 now requires its recorded G4 A host hashes to equal the G4 A bundle's, and a disposable copy with a changed hash fails by name. The G4 A and G6 input checks require the input file among the G3 bundle's sources. Every changed importer was rerun against the closed workspace. Unit tests, every verifier, `summarize.py` and the figure check pass. No device was run.

## G8 import — 2026-09-16

The importer ran twice into new directories and produced byte-identical bundles. It recounts every admission's ANE requests from the host-PID logs under both clock mappings, checks both phases' launch records, the follow-up's job selection and each block's clock order, count and recorded p50, and refuses a mismatch. The verifier checks every asset audit, re-applies the admission rules to all <!-- claim:g8.configs@g8-065 -->20<!-- /claim --> prepared configurations, the <!-- claim:g8.rejected@g8-066 -->3<!-- /claim --> rejected admissions and every block's own controls, recomputes <!-- claim:g8.blocks@g8-067 -->60<!-- /claim --> blocks from their windows, call counts and <!-- claim:g8.power-samples@g8-068 -->10,625<!-- /claim --> power samples, and compares speed, latency, energy at four lags, admission and idle power with what the run recorded. Disposable-copy tests change a decoded-weight hash, a saved operator count, a relative L2, a recount under the phase's mapping, a Mach-clock recount, a follow-up block's mapping, a rejected admission's request counts, the follow-up inventory, a power frame's CPU energy together with its decoded record, a recorded speed and a recorded idle power, and each fails by name. Missing numeric controls, missing all/ordinary comparisons and missing, empty or truncated Mach-request lists also fail by name. The maximum relative L2 below is derived from prepared, timed and rejected admissions and bound to the document checker.

Before the import, the closed workspace was checked from its raw files, which are not in this repository: the four Core ML packages' constants were decoded with NumPy and are bitwise equal to each other and to the codes times the FP16 scale; the output tensors of every admission and block were compared with the CPU references (largest relative L2 <!-- claim:g8.max-relative-l2@g8-069 -->0.00218<!-- /claim -->) and the byte-identity classes confirmed; both clock mappings were reimplemented from the logs and reproduce every recorded count. No device was run for this import.
