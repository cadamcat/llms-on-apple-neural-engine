...
限定条件：运行中有意外加载的动态屏保（收尾后才发现）；六个共存组中有五组的起始温度和风扇状态不匹配；功率采集
...
<td><b><!-- claim:g2.native-calls@calls-card -->33,728 次调用<!-- /claim --></b><br><sub>每个原生 ANE 宿主的内存 footprint 在结束时比开始时低 <!-- claim:g2.native-shrink@memory-card -->60–62 MiB<!-- /claim --></sub></td>
...
2. **Core AI 原生 LUT 探针算错。** 输出与实验前固定的 flattened-scale 错误预测一致；
...
- **严格的跨模型逐字节一致性检查未通过。** E2B 完全一致，Qwen8B 余 13 处残差。
...
后续服务实验再测 GPU 风扇在 4.9 到 23.7 请求/s 之间从哪里开始升高，先覆盖 4.9 到约 6.4 请求/s；以及前台矩阵计算任务的尾延迟优势在关闭动画、匹配起始温度和风扇状态并加入只占 CPU 的忙等基线后是否仍然存在。[RESEARCH.md](docs/RESEARCH.md) · [RELATED_WORK.md](docs/RELATED_WORK.md)
