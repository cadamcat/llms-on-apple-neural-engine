...
在 LLM 推理中，分组量化常被描述为权重存储格式：每若干输入通道共享一个 scale，代码与 scale 相乘后参与矩阵乘法。但在 Apple 推理栈中，必须分开问：资产是否正确、编译器是否接受这种 scale 粒度、运行时是否真的让算子参与 ANE。导出和 CPU 数值正确，不意味着会被 ANE 执行。
...
图等价不等于浮点执行等价。拆分参考在部分和、平衡树和层间 QDQ 处有 FP16 边界，宽 K 路径则不同，必须使用各自冻结的参考。见[方法文档](../../docs/METHODS.md)。
...
这只证明：固定图、芯片和工具链上的低比特表示能够形成 ANE 正例。低熵 ±1 权重和易表达 scale 不能外推到任意 Q4_0、LLM throughput 或能效；Core AI 无逐算子执行追踪，且 `physical_INT8_proven=false`。
...
这不是“分组量化必然慢四倍”，而是一个图结构观察。K32 拆分增加图内卷积操作、部分和与加法树，并改变融合、调度、窄 K 内核及 FP16 归约边界。现有测量没有逐项隔离这些因素，只能说完整拆分图在此主机上显著变慢。两条路径采用各自冻结的数值参考；同权重并不代表浮点输出逐位相同。
...
图 2：连线连接同一轮的宽/拆分配置，不代表随时间变化。横轴从零开始，拆分 p50 除以宽 K p50 得到宽路径相对拆分的速度。历史单对照 4.44 倍没有混入新测三个配对。[逐点数值](../../docs/figures/manifest.json)。
...
把结果压缩为工程判断如下。表中的“假说”是待检验解释，不是本轮证明的事实。
...
静态解码不能证明设备执行，设备事件不能证明物理指令是 INT8，数值通过也不能证明模型质量；失败结果标记了不能作为 benchmark 臂的表示。
